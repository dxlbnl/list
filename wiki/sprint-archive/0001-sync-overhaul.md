...
**Outcome:** all 13 cards done. Sync is correct (no delete-resurrection / batch data-loss / authz spoof /
slug-abort), non-stalling (gentle backoff — no 20s cliff — + poison-op quarantine), and converges
cross-device in **one round-trip** via the `updated_seq` cursor delta, with row-level HLC LWW enforced on
both surfaces. 25 tests, `check`/`lint` green throughout.

**What worked:** extracting `processSyncBatch` (server) + `createSyncManager`/`ListDatabase(name)` (client)
made the whole engine node-tier testable with fake-indexeddb + pglite — **no chromium**. The two-client
harness proved A→B convergence. Strict red→green TDD on every server bug.

**Snags & fixes:** exporting the `$state` class tripped `svelte/prefer-svelte-reactivity` → kept the class
private, exposed a factory. A pure-unit test sharing a pglite `afterEach` double-closed the db → null the
handle after close.

**Follow-ups (NOT in this sprint):** apply `drizzle/0001` to Neon (`pnpm db:migrate` / `db:push`) — the app's
`/api/sync` errors until then; rank renormalisation on float exhaustion; per-field LWW only if different-field
loss appears; the non-sync backlog (persistent-groups, gift-lists, group quirks, rate-limit removal, …).

**Learnings captured as atoms:** sync-model (rewritten to the new flow), sync-redesign, sync-merge-model
(HLC), sync-latency, async-sync-testing, server-modules, data-model.
