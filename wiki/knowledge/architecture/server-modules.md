...
- **`POST /api/sync`** (`src/routes/api/sync/+server.ts`) — the only complex endpoint. Validates the batch with the Zod `syncRequestSchema`, then applies **all** operations (item/list upserts + deletes) in **one atomic CTE** against Postgres: `jsonb_populate_recordset` dedups input, upserts enforce membership authorization (`list_users`), item upsert is guarded by `updated_at < EXCLUDED.updated_at` (last-write-wins), and it `RETURNING`s written ids so the response reports per-op `success`/`ignored`. One request → one round-trip. It is **push-only — there is no GET/SSE**; pull is Supabase Realtime (see [sync-model](sync-model.md)).
