...
**Loop / reconnect.** `startLoop()` runs `processQueue()` every ~2 s and `pullDelta()` ~every 10 s (Realtime is the fast path, so the backfill poll is infrequent; gentle jittered backoff capped at 30 s on error — no 20 s cliff; a batch that fails 5× is quarantined) and `reconcileAllLists()` ~every 60 s (the authoritative list set). `online`/`visibilitychange` also push + reconcile + `pullDelta`. The Supabase JWT (1 h) refreshes 5 min before expiry and on `CHANNEL_ERROR`.
